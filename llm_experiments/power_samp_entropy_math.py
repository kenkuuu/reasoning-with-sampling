...
from power_samp_utils import (
    AutoregressiveSampler, get_kv_cache,
    entropy_guided_block_mcmc, naive_temp, format_prompt,
)
...
if __name__ == "__main__":
    ...
    print(f"temp={args.temperature} (alpha={1/args.temperature:.2f})  block_size={args.block_size}")
    print(f"tau_avg={args.tau_avg}  tau_max={args.tau_max}  beta={args.beta}  mcmc_steps={args.mcmc_steps}")
    ...
    for problem, data in tqdm(enumerate(dataset[start:end]), desc="MATH entropy MCMC"):
        ...
        eg_ids  = torch.tensor(eg_output, dtype=torch.long, device=device).cpu()
        ...
        print(f"[{problem}] std={std_answer}  eg={eg_answer}  correct={answer}  accept={acceptance_ratio:.3f}")
    ...
    fname = (f"{model}_math_entropy_mcmc"
